...
# (Your imports, commands, and logic remain exactly as before)
...
async def start(interaction: discord.Interaction):
    ...
    if msg.content.lower() == "server owner":
        config[guild_id]["ticket_recipient"] = interaction.guild.owner.id
    else:
        try:
            user_id = int(msg.content)
            user = interaction.guild.get_member(user_id)
            if user:
                config[guild_id]["ticket_recipient"] = user.id
            else:
                await interaction.followup.send("⚠️ User not found. Defaulting to server owner.")
                config[guild_id]["ticket_recipient"] = interaction.guild.owner.id
        except ValueError:
            await interaction.followup.send("⚠️ Invalid ID. Defaulting to server owner.")
            config[guild_id]["ticket_recipient"] = interaction.guild.owner.id
    ...
    msg = await ask_question(
        interaction,
        "✅ What is your **verification channel**?\nOptions: Channel ID or mention, `create` one for me, or `none`.",
        check
    )
    if msg is None:
        return
    ...
    if msg.content.lower() == "none":
        config[guild_id]["verification_channel"] = None
    elif msg.content.lower() == "create":
        verification_channel = await interaction.guild.create_text_channel("verification")
        config[guild_id]["verification_channel"] = verification_channel.id
    else:
        try:
            verification_channel = interaction.guild.get_channel(int(msg.content.strip("<#>")))
    ...
    # Final setup completion embed
    await log_channel.send(
        embed=discord.Embed(
            title="✅ Setup Complete",
            description=(
                f"Bot is now configured for `{interaction.guild.name}`.\n\n"
                f"**Bot Logs Channel:** {config[guild_id]['bot_logs'] or 'None'}\n"
                f"**Verification Channel:** {config[guild_id]['verification_channel'] or 'None'}\n"
...
#########################################################################################################
    ...
    # If a verification channel exists, send the verification message
    if config[guild_id]["verification_channel"]:
        try:
            if not verification_channel:  # Fetch if not already set above
                verification_channel = interaction.guild.get_channel(config[guild_id]["verification_channel"])
            if verification_channel:
                ...
                # Save verification message ID for reaction tracking
                config[guild_id]["verification_message_id"] = verify_msg.id
                save_config(config)

        except discord.Forbidden:
            await interaction.channel.send(
                f"⚠️ I couldn't send a verification message in {verification_channel.mention} due to missing permissions."
            )
...
async def on_raw_reaction_add(payload):
    """Handles verification reactions."""
    ...
    # Check if this guild has verification configured
    if guild_id not in config:
        return

    verification_channel_id = config[guild_id].get("verification_channel")
    verification_message_id = config[guild_id].get("verification_message_id")
    verified_role_id = 1404908934287982602  # The role to give

    if not verification_channel_id or not verification_message_id:
        return
    ...
    try:
        ...
        pass  # Bot doesn't have permission to add the role
...
class TicketModal(discord.ui.Modal, title="Create Ticket 📨"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            await interaction.response.send_message(
                "✅ Your ticket has been submitted to the server owner.",
                ephemeral=True
            )
        except discord.Forbidden:
            await interaction.response.send_message(
                "⚠️ Could not send ticket to the server owner (DMs closed).",
                ephemeral=True
            )
...
class TicketButton(discord.ui.View):
    ...
    async def create_ticket(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if now - last_used < 600:  # 10 minutes cooldown
            remaining = int(600 - (now - last_used))
            return await interaction.response.send_message(
        ...
        await interaction.response.send_modal(TicketModal(self.owner))
...
async def ticket(interaction: discord.Interaction):
    ...
    embed = discord.Embed(
        ...
        description=(
            ...
            "Click the **Create Ticket** button below to send a message directly to the server owner.\n"
...
async def userroles(interaction: discord.Interaction, user: discord.Member):
    ...
    embed.set_footer(text=f"Total Roles: {len(user.roles) - 1}")
    await interaction.response.send_message(embed=embed)
...
@giverole.error
@removerole.error
async def role_command_error(interaction: discord.Interaction, error):
    if isinstance(error, app_commands.MissingPermissions):
...
async def viewroles(interaction: discord.Interaction):
    roles = [role.mention for role in interaction.guild.roles if role != interaction.guild.default_role]
    roles.reverse()  # Highest role first

    if not roles:
        roles_str = "No roles found."
    else:
        roles_str = ", ".join(roles)
...
# ... (all your current commands and events remain exactly the same) ...

# ---------- Run ----------
if __name__ == "__main__":
    token = os.getenv("BOT_TOKEN")  # Retrieve from Replit's environment variables
    if not token:
        raise SystemExit("❌ Please set BOT_TOKEN environment variable in Replit Secrets.")
    
    keep_alive()  # Start the web server for uptime pings
    bot.run(token)
